...
| `s_frac_arc`  | (Nf,)  | **arc length** along Γ from `A` to each fracture cell |

`Nm = 16384` (matrix cells), `Nf = 125` (fracture cells).

> ⚠️ **Renamed:** arc length is now `s_frac_arc`. `s_frac` / `s_matrix` now hold the
> MRST-native **saturation** fields (section 4), per the transport spec.
...
| field | shape | meaning |
|---|---|---|
| `s_matrix` | (Nm,) | matrix water saturation at PVI=`meta_PVI` |
| `s_frac`   | (Nf,) | fracture water saturation at PVI=`meta_PVI` |

Produced with the **same scheme as the notebook**: explicit first-order upwind,
`f(S)=S²/(S²+(1−S)²)`, `CFL=0.45`, `FPRIME_MAX=2`, matrix-PV basis for PVI,
`T_final=0.47938`, `nsteps=13384`, `dt=3.58e-5` (all matching the notebook MRST run).
Results: matrix Sw∈[0,0.9955], fracture Sw∈[0,0.2126].

**Validate:** RMSE of the notebook's rect-MRST `S_Γ`/`S` against `s_frac`/`s_matrix`
— should be ~discretization error (scheme is identical). If large, suspect (a) along-flux
source — use `frac_face_flux` not a `p_frac` gradient; (b) storage convention — fracture
storage is **aperture-free** `φ·ℓ`; (c) tip-flux sign.
