...
| `meta_T_final` (PVI=1.2) | 0.47938 | **0.34653** (recomputed) |
| `meta_nsteps` | 13384 | 2727 |
| fracture `sw_frac` max | 0.2126 (oil conduit) | **0.8114** (floods with water) |
| matrix `sw_matrix` max | 0.9955 | 0.9956 |
| `meta_tip_bc` | (Dirichlet wells) | `'no-flow ... fracture pressure floats'` |

`sw_*` are again from MRST's independent `explicitTransport` (corroborated vs the
hand-coded scheme to RMSE 3.1e-4 matrix / 3.8e-4 fracture). All fluxes (`face_flux`,
