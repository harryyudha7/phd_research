%% SPE10 layer 20 (Tarbert) RESAMPLED to 64x64, pressure + transport.
%% Same as case3_spe10_L20_128.m but on a 64x64 grid (coarser).
...
%% Hand-code + MRST explicitTransport. Writes ONLY case3_mrst_export_spe10_L20_64.mat.
...
%% SPE10 layer 20 -> normalize (geomean=1 over full layer) -> resample 60x220 to NGxNG (nearest)
...
kappa_grid=Korig(ispe,jspe);                          % NGxNG nearest-neighbor
kappa_cell=kappa_grid(:);                             % cartGrid([NG NG]) ordering
fprintf('\n=== SPE10 L%d resampled to %dx%d ===\n',LAYER,NG,NG);
...
meta_source=sprintf('SPE10 model-2 layer %d (Tarbert), Kx normalized geomean=1, resampled 60x220 -> %dx%d nearest',LAYER,NG,NG);
meta_grid=sprintf('cartGrid([%d %d]) unit square; kappa_grid(i,j): i=x, j=y',NG,NG);
...
meta_Q_in=Qin; meta_PV=PV; meta_T_pvi03=T1; meta_T_pvi05=T2; meta_NG=NG;
meta_transport_solver='MRST explicitTransport (sw_*) ; hand-coded upwind (sw_*_matched)';
README=sprintf('SPE10 L%d (Tarbert) resampled %dx%d. kappa_grid=%dx%d normalized perm. face_flux=MRST conservative flux. sw_matrix_pvi03/05=Sw at PVI 0.3/0.5. For CG: %dx%d mesh, load kappa_grid as DG0, BCs p=1/p=4.',LAYER,NG,NG,NG,NG,NG,NG);
save('c:\Users\muchamad\mrst-project\case3_mrst_export_spe10_L20_64.mat','-v7', ...
  ...
  'meta_T_pvi03','meta_T_pvi05','meta_NG','meta_transport_solver','README');
fprintf('saved case3_mrst_export_spe10_L20_64.mat\n');

figure('Name','SPE10 L20 64 log10 kappa','Position',[60 80 500 460]);
plotCellData(G,log10(kappa_cell),'EdgeColor','none'); colormap(jet); view(0,90); axis equal tight; colorbar;
title('SPE10 L20 (64^2)  log_{10}\kappa'); xlabel x; ylabel y;
figure('Name','SPE10 L20 64 Sw PVI=0.3','Position',[580 80 500 460]);
plotCellData(G,s_e_t1,'EdgeColor','none'); colormap(flipud(winter)); caxis([0 1]); view(0,90); axis equal tight; colorbar;
title(sprintf('SPE10 L20 (64^2)  S_w at PVI=0.3 (T=%.4f)',T1)); xlabel x; ylabel y;
